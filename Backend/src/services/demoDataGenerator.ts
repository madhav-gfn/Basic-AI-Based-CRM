...
// ─────────────────────────────────────────────────────────────────────────────
// generateDemoOrgData — builds a full, realistic dataset (customers, orders,
// segments, campaigns, A/B variants, communications + events, journeys +
// enrollments) scoped entirely to one existing organization.
//
// This is the shared engine behind both `Prisma/seed.ts` (the CLI seed,
// which wipes and rebuilds the whole database for local dev) and the
// `/api/demo/seed` endpoint (which only ever creates data for a brand-new,
// dedicated demo org — it never deletes anything). Every row this function
// creates is written with `organizationId: orgId`, so it's safe to call
// against an org that has no prior data without touching any other tenant.
// ─────────────────────────────────────────────────────────────────────────────
...
export interface DemoDataOptions {
  /** Number of synthetic customers to generate. Default 2000 (full CLI seed). */
...
export async function generateDemoOrgData(
  orgId: string,
  options: DemoDataOptions = {}
): Promise<DemoDataResult> {
  const customerCount = options.customerCount ?? 2000;

  // ── Customers + Profiles ──────────────────────────────────────────
  const customerProfiles: CustomerProfile[] = [];
  const customerData = Array.from({ length: customerCount }, (_, i) => {
    const persona = pickPersona();
    const customer: CustomerProfile = {
      id: randomUUID(),
      name: `Customer ${i + 1}`,
      email: `customer${i + 1}-${orgId.slice(0, 8)}@example.com`,
  ...
  for (const profile of customerProfiles) {
    ...
    switch (profile.persona) {
      case "VIP":
        orderCount = Math.floor(Math.random() * 13) + 10;
        break;
      case "REGULAR":
        orderCount = Math.floor(Math.random() * 5) + 4;
        break;
      case "DORMANT":
      ...
      case "DISCOUNT_HUNTER":
        orderCount = Math.floor(Math.random() * 7) + 3;
        break;
    }
    ...
    for (let i = 0; i < orderCount; i++) {
      ...
      if (!profile.firstOrderDate || orderDate < profile.firstOrderDate) profile.firstOrderDate = orderDate;
      if (!profile.lastOrderDate || orderDate > profile.lastOrderDate) profile.lastOrderDate = orderDate;
    }
  ...
  const segmentDefinitions: Array<{ name: string; definition: SegmentDefinition }> = [
    ...
    },
    {
      ...
      definition: { operator: "AND", rules: [{ field: "daysSinceLastOrder", op: "gte", value: 90 }] },
    },
    {
      name: "New Customers",
      definition: {
    ...
    {
      name: "Delhi Shoppers",
      definition: { operator: "AND", rules: [{ field: "city", op: "eq", value: "Delhi" }] },
    },
    {
    ...
    },
    {
      ...
      definition: { operator: "AND", rules: [{ field: "orderCount", op: "gte", value: 8 }] },
    },
    {
    ...
    },
    {
      ...
      definition: { operator: "AND", rules: [{ field: "favoriteCategory", op: "eq", value: "Footwear" }] },
    },
    {
      name: "Beauty Buyers",
      definition: { operator: "AND", rules: [{ field: "favoriteCategory", op: "eq", value: "Beauty" }] },
    },
    {
  ...
  const campaigns = await Promise.all(
    campaignsSeed.map(async (campaignSeed) => {
      ...
      if (campaignSeed.status === CampaignStatus.COMPLETED || campaignSeed.status === CampaignStatus.RUNNING) {
    ...
    [variantA, variantB] = await Promise.all([
      prisma.campaignVariant.create({
        data: {
          ...
          label: "A",
          message: "We miss you! Come back and enjoy 20% off on your favourite styles \u{1F6CD}\u{FE0F}",
          weight: 1,
        },
      }),
      prisma.campaignVariant.create({
  ...
  function eventTimeline(base: Date, stage: "success" | "fail") {
    ...
    if (clickRoll < 0.22)
      events.push({ eventType: EventType.CLICKED, timestamp: clickedAt, metadata: { url: "/collections/new-arrivals" } });
    if (convertRoll < 0.05)
      events.push({ eventType: EventType.CONVERTED, timestamp: convertedAt, metadata: { orderValue: randomFloat(999, 7999) } });
  ...
  await prisma.communication.createMany({ data: communicationsToInsert });
  await prisma.communicationEvent.createMany({ data: eventsToInsert });
